...
interface SongPlayCountsByDifficultyTableProps {
  songPlayCounts: SongPlayByDifficulty[]
}

export default function SongPlayCountsByDifficultyTable({ songPlayCounts }: SongPlayCountsByDifficultyTableProps) {
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 border border-gray-200 dark:border-gray-700">
      <div className="flex justify-between items-center mb-4">
        ...
        <span className="text-sm text-gray-600 dark:text-gray-400">
          全 {songPlayCounts.length} 曲
        </span>
      </div>
      <div className="text-xs text-gray-500 dark:text-gray-400 mb-2">
        同じユーザーが複数回プレイした場合も全てカウントされます
      </div>
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {songPlayCounts.map((song, index) => {
              // 新曲を強調表示（合計プレイ回数が50回以下）
              const isNewSong = song.total <= 50
              return (
                <tr
                  key={index}
