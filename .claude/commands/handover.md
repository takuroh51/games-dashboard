...
1. **実験ディレクトリを解析**:
   ```bash
   # アクティブな実験数
   ls .claude/experiments/active/ | wc -l

   # 成功した実験数
   ls .claude/experiments/completed/ | wc -l

   # 失敗した実験数
   ls .claude/experiments/failed/ | wc -l

   # パターン数
   ls .claude/proven_patterns/ | wc -l
   ```

2. **最近の実験を抽出**:
   - `active/`, `completed/`, `failed/` から最新5件を取得
   - 各実験のタイトルとステータスを取得

3. **パターン一覧を収集**:
   - `proven_patterns/` から全パターンを取得
   - パターンID、タイトル、信頼度、適用回数を抽出

4. **統計情報をセッション記録に含める**:
   セッション記録の冒頭に以下のセクションを追加：
   ...
   **最近の実験**:
   1. [exp_ID] - [タイトル] (ステータス: active/completed/failed)
   2. [exp_ID] - [タイトル] (ステータス: active/completed/failed)
   ...
   ...
   **パターン一覧**:
   1. [pat_ID] - [タイトル] (信頼度: 高/中/低, 適用回数: N回)
   2. [pat_ID] - [タイトル] (信頼度: 高/中/低, 適用回数: N回)
   ...
   ```
